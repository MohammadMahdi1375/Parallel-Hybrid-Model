...
parser.add_argument("--d_model",                        default=2048,            help="Model dimentionality (d_model=hidden_size)")
parser.add_argument("--MLP_intermediate_size",          default=8192,           help="diemntionality of the inn and out in the gate, up, and down projections")
...
parser.add_argument("--N_Hybrid_Layers",                default=6,              help="Number of Blocks in the model")
...
parser.add_argument("--TOKEN_LIMIT",                    default=2_000_000_000,  help="Number of tokens we want to train our model on")
parser.add_argument("--BATCH_SIZE",                     default=4,              help="Batch size")
parser.add_argument("--gradient_accumulation_steps",    default=16,              help="Gradient Accumulation Step")
parser.add_argument("--LR",                             default=1e-4,           help="Learning Rate")
parser.add_argument("--WEIGHT_DECAY",                   default=0.1,           help="weight decay in the optimizer")
parser.add_argument("--betas",                          default=(0.9, 0.999),           help="weight decay in the optimizer")
parser.add_argument("--DEVICE",                         default='cuda',       help="Device Type")
parser.add_argument("--model_adr",                      default='SHM_1B',     help="Folder in which we store the final model")
parser.add_argument("--Parallel_Mode",                  default='A',            help="Folder in which we store the final model")

args = parser.parse_args()

total_token_count = 0
tensorboard_writer = SummaryWriter(log_dir="runs_1B/" + args.model_adr)
##########################################################################################################################################################################
############################################################################# Model Declration ###########################################################################
##########################################################################################################################################################################
...
for idx, data in progress_bar:
    ...
    if total_token_count < args.TOKEN_LIMIT:
        ...
        if (idx % args.gradient_accumulation_steps == 0):
            ...
            end_event = time.time()
            if (idx > int(0.1 * num_training_steps/args.gradient_accumulation_steps)):
                token_count += torch.sum(inputs['attention_mask'] == 1).item()
        ...
        if idx % int(0.05 * num_training_steps) == 0:
            # with open("./saved_model/" + args.model_adr + "/model.pth", "wb") as f:
            #     dill.dump(model, f) 
            torch.save(model, "./saved_model/" + args.model_adr + "/model.pth")
...
print("==========================================================================================")
print(f"Token/Sec is: {round(token_count / gpu_total_s, 3)}")
print(f"Iter/Sec is: {round(gpu_steps / gpu_total_s, 3)}")
...
if (flops_splits[1] == "GFLOPS"):
    print(f"MFU is: {float(flops_splits[0]) / (avg_gpu_time * 309.7 * 1000)}")
elif (flops_splits[1] == "TFLOPS"):
    print(f"MFU is: {float(flops_splits[0]) / (avg_gpu_time * 309.7)}")
