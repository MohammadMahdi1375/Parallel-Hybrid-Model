...
class MambaAttentionDecoderLayer(nn.Module):
    def __init__(self, config: JambaConfig, layer_idx: int):
        ...
        self.config = config
        self.layer_idx = layer_idx
        self.mamba = JambaMambaDecoderLayer(config=self.config, layer_idx=layer_idx)
        self.atten = JambaAttentionDecoderLayer(config=self.config, layer_idx=layer_idx)

        if self.config.Prallel_Mode == 'A':
            self.branch_proj = nn.Linear(2*self.config.hidden_size, self.config.hidden_size, bias=False)
        else:
            self.branch_proj = nn.Linear(self.config.hidden_size, self.config.hidden_size, bias=False)

        self.pre_ff_layernorm = JambaRMSNorm(self.config.hidden_size, eps=self.config.rms_norm_eps)
        self.final_proj = nn.Linear(self.config.hidden_size, self.config.hidden_size, bias=False)
    ...
    def forward(
        ...
        if self.config.Prallel_Mode == 'A':     ##### No Splitting
            mamba_mask, atten_mask = attention_mask[0], attention_mask[1]
            ...
            e1 = time.time()
            s2 = time.time()
            atten_output = self.atten(hidden_states,
            ...
            e2 = time.time()
            # print("\n###########################################################################")
            # print(f"# Block index: {self.layer_idx}, Mamba: {round(e1-s1, 8)}, Attention: {round(e2-s2, 8)}, ration:{round((e1-s1)/(e2-s2), 4)}, {hidden_states.shape[1]} #")
            # print("###########################################################################")

            mamba_atten_output = torch.cat((mamba_output[0], atten_output[0]), dim=2)
            mamba_atten_output = self.branch_proj(mamba_atten_output)
            mamba_atten_output = residual + mamba_atten_output

            residual = mamba_atten_output
            mamba_atten_output = self.pre_ff_layernorm(mamba_atten_output)
            mamba_atten_output = self.final_proj(mamba_atten_output)
            self_attn_weights = None
            hidden_states = residual + mamba_atten_output
            outputs = (hidden_states,)

        elif self.config.Prallel_Mode == 'B':     ##### Alternate Mode
            if self.layer_idx%2 != 0:
                half_part = hidden_states.shape[1]//2
                residual = hidden_states
                mamba_inp = hidden_states[:, half_part:, :]
                atten_inp = hidden_states[:, :half_part, :]

                if attention_mask[0] != None:
                    mamba_mask = attention_mask[0][:, half_part:]
                else:
                    mamba_mask = None
                if attention_mask[1] != None:
                    atten_mask = attention_mask[1][:, :, :half_part, :half_part]
                else:
                    atten_mask = None

                mamba_output = self.mamba(mamba_inp,
                                        attention_mask=mamba_mask,
                                        position_ids=position_ids,
                                        past_key_value=past_key_value,
                                        output_attentions=output_attentions,
                                        ...
                                        use_cache=use_cache,
                                        cache_position=cache_position)
                atten_output = self.atten(atten_inp,
                                        attention_mask=atten_mask,
                                        position_ids=position_ids,
                                        past_key_value=past_key_value,
                                        output_attentions=output_attentions,
                ...
                mamba_atten_output = torch.cat((atten_output[0], mamba_output[0]), dim=1)
                mamba_atten_output = self.branch_proj(mamba_atten_output)
                mamba_atten_output = residual + mamba_atten_output
                ...
                mamba_atten_output = torch.cat((mamba_output[0], atten_output[0]), dim=1)
                mamba_atten_output = self.branch_proj(mamba_atten_output)
                mamba_atten_output = residual + mamba_atten_output
        ...
        elif self.config.Prallel_Mode == 'C':     ##### Flop-Aware Mode
            devision_part = int(0.59 * hidden_states.shape[1])

            residual = hidden_states
            mamba_inp = hidden_states[:, devision_part:, :]
            atten_inp = hidden_states[:, :devision_part, :]

            if attention_mask[0] != None:
                mamba_mask = attention_mask[0][:, devision_part:]
            else:
                mamba_mask = None
            if attention_mask[1] != None:
                atten_mask = attention_mask[1][:, :, :devision_part, :devision_part]
            else:
                atten_mask = None
            ...
            # print("\n###########################################################################")
            # print(f"# Block index: {self.layer_idx}, Mamba: {round(e1-s1, 8)}, Attention: {round(e2-s2, 8)}, ration:{round((e1-s1)/(e2-s2), 4)} #")
            # print("###########################################################################\n")
            mamba_atten_output = torch.cat((atten_output[0], mamba_output[0]), dim=1)
            mamba_atten_output = self.branch_proj(mamba_atten_output)
            mamba_atten_output = residual + mamba_atten_output
        ...
        elif self.config.Prallel_Mode == 'D':     ##### Circulating Mode
            if self.layer_idx%2 != 0:
                devision_part = int(0.5 * hidden_states.shape[1])
                ...
                devision_part = hidden_states.shape[1] - int(0.5 * hidden_states.shape[1])
...
ALL_PARALLEL_BLOCKS= {"paralell_Attn_SSM": MambaAttentionDecoderLayer}
...
class JambaModel(JambaPreTrainedModel):
    ...
    def __init__(self, config: JambaConfig):
        ...
        #################################################################################################################################### Modified
        for i in range(config.num_hidden_layers):
            # ayer_class = ALL_DECODER_LAYER_TYPES[config.layers_block_type[i]]
            # decoder_layers.append(layer_class(config, layer_idx=i))
            decoder_layers.append(MambaAttentionDecoderLayer(config, layer_idx=i))
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            layer_mask = [mamba_mask, causal_mask]
            #######################################################################################################################################
